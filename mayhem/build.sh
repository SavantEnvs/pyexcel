...
PKGS=(
  ...
  # tests/requirements.txt pins the original 'nose' for eq_/raises, but it imports the 'imp'
  # module removed in Python >= 3.12; pynose is a maintained fork installed under the same
  # 'nose' package name, so `from nose.tools import ...` in tests/ keeps working unmodified.
  pynose
)
need_download=0
...
# tests/test_examples.py imports the stdlib 'imp' module (removed in Python >= 3.12) directly for
# its single load_source() call — shim it into the site dir (found via PYTHONPATH, since the real
# module no longer exists) rather than touching the upstream test file.
if [ ! -f "$SITE/imp.py" ]; then
  cat > "$SITE/imp.py" <<'PYEOF'
...
# tests/ was written for `nosetests` (test.sh -> nosetests), which calls setUp()/tearDown() on
# PLAIN classes (no unittest.TestCase base) the same as it does on TestCase subclasses. pytest's
# own nose-compat shim never recognized that camelCase pair on plain classes (only the lowercase
# setup()/teardown()), and pytest >= 8 dropped nose-compat entirely — so tests/test_sheet_row.py
# et al. silently skip their setUp() and fail on missing attributes. A generated (non-committed)
# conftest.py restores exactly that nose semantic; TestCase subclasses are left to unittest's own
# integration to avoid calling setUp() twice.
if [ ! -f "$SRC/conftest.py" ]; then
  cat > "$SRC/conftest.py" <<'PYEOF'
import unittest
...
def _nose_style_setup_teardown(request):
...
# tests/test_stringio.py's two sys.stdout round-trip tests rely on sys.stdout itself exposing
# .getvalue() (true under nosetests' own stdout capture). pytest's default fd-level capture
# replaces sys.stdout with an EncodedFile that has no .getvalue(); its sys-level capture mode
# (CaptureIO) does. A generated (non-committed) pytest.ini pins that mode without touching the
# committed test files.
if [ ! -f "$SRC/pytest.ini" ]; then
  cat > "$SRC/pytest.ini" <<'PYEOF'
[pytest]
addopts = --capture=sys
PYEOF
fi
